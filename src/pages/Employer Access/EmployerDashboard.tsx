              ...
              <div key={application.id} className="bg-white border border-gray-200 rounded-xl p-6 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(0,0,0,0.05)] hover:border-blue-500">
                ...
                <div className="flex gap-3 flex-wrap">
                  {application.user?.resumeUrl && (
                    <>
                      <button className="flex items-center gap-2 py-2.5 px-4 rounded-lg text-sm font-medium w-auto h-auto bg-blue-50 text-blue-600 border border-blue-200 hover:bg-blue-100 transition-all duration-300" onClick={() => window.open(`${process.env.REACT_APP_API_URL || 'https://mcb.instatripplan.com'}${application.user.resumeUrl}`, '_blank')}>
                        <FileText className="w-4 h-4" />
                        <span>View Resume</span>
                      </button>
                       <a 
                         href={`${process.env.REACT_APP_API_URL || 'https://mcb.instatripplan.com'}${application.user.resumeUrl}`}
